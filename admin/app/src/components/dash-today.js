...
import { getRealTime } from "../helpers/realtime";

class DashToday extends Component {
  ...
  componentDidMount() {
    let start_of_day = dayjs().startOf('day').format('X');
    let time_now = dayjs().format('X');

    this.get_four_stats(start_of_day, time_now);
    this.get_top_pages(start_of_day, time_now);

    this.get_real_time_users();
  }

  async get_real_time_users() {
    getRealTime().then((count) => {
      this.setState({
        realtime: count
      })
    })
  ...
  render() {
    const { stat_pv, desktop_percent, mobile_percent, topPages, topPagesLoading, realtime } = this.state;
    ...
    return (
      <div id="dash-today">
        <Row gutter={[15, 15]} className="ckls-dash-stats">
          <Col span={12}>
            <Card>
              <Statistic title="Real-Time" value={realtime} precision={0} valueStyle={{ color: 'red' }} />
            </Card>
          </Col>
          <Col span={12}>
            <Card>
        ...
        <Row>
          <Col span={24}>
            <Card title="Trending" bordered={false} size="small" className="ckls-trending-pages" loading={topPagesLoading}>
              {
                (topPages.length === 0) ?
                  'No pages found for today. Please check back later.' :
